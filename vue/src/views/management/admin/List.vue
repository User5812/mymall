    ...
    <el-table :data="tableData" stripe>
      ...
      <el-table-column label="操作">
        <template v-slot="scope">
          ...
          <el-button type="warning" @click="handleChangePass(scope.row)">修改密码</el-button>
        </template>
      </el-table-column>
    </el-table>
  ...
  <el-pagination
    ...
    :total="total">
      ...
      <el-dialog title="修改密码" :visible.sync="dialogFormVisible">
        <el-form :model="form" :label-width="formLabelWidth" ref="formRef" :rules="rules" >
          <el-form-item label="密码" prop="newPass">
            <el-input style="width: 600px;" show-password v-model="form.newPass" autocomplete="off"></el-input>
          </el-form-item>
        </el-form>
        ...
        if(valid){
          request.put('/admin/password',this.form).then(res => {
            if (res.code === '200'){
              ...
              if(this.form.id === this.admin.id){ //当前修改的用户id等于当前登录的管理员id，那么修改成功之后重新登录
